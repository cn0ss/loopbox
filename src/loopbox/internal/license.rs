...
fn resolve_device_identifier() -> Option<String> {
    if let Some(override_device_id) = optional_env("LOOPBOX_LICENSE_DEVICE_ID") {
        return normalize_device_identifier(override_device_id);
    }

    #[cfg(target_os = "macos")]
    {
        if let Some(identifier) = read_macos_platform_uuid() {
            return normalize_device_identifier(identifier);
        }
    }

    #[cfg(target_os = "linux")]
    {
        if let Some(identifier) = read_linux_machine_id() {
            return normalize_device_identifier(identifier);
        }
    }
    ...
    {
        if let Some(identifier) = read_windows_machine_guid() {
            return normalize_device_identifier(identifier);
        }
    }
...
fn normalize_device_identifier(raw: String) -> Option<String> {
...
fn read_macos_platform_uuid() -> Option<String> {
    ...
    for line in stdout.lines() {
        if line.contains("IOPlatformUUID") {
            if let Some(value) = parse_quoted_value_after_equals(line) {
                return Some(value);
            }
        }
    }
...
#[cfg(target_os = "macos")]
fn parse_quoted_value_after_equals(line: &str) -> Option<String> {
