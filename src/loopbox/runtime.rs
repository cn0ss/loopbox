...
fn run_runtime_pty_runner(args: RuntimePtyRunnerArgs) -> Result<i32, String> {
    ...
    {
        let _ = args;
        return Err("Runtime PTY runner is only available on Unix hosts.".to_string());
    }
    ...
    #[cfg(unix)]
    {
        ...
        let command_arg = CString::new(args.command.as_str())
            .map_err(|_| "Runtime PTY command contains an unsupported NUL byte.".to_string())?;
        let workdir = CString::new(args.workdir.as_str())
        ...
        let input_thread =
            thread::spawn(move || forward_fifo_input_to_pty(input_path, writer_fd, input_stop));
...
#[cfg(unix)]
fn wait_for_child_exit(pid: libc::pid_t) -> Result<i32, String> {
...
fn run_runtime_attach_bridge(args: RuntimeAttachBridgeArgs) -> Result<i32, String> {
    ...
    {
        let _ = args;
        return Err("Runtime attach bridge is only available on Unix hosts.".to_string());
    }
    ...
    #[cfg(unix)]
    {
        ...
        let output_thread =
            thread::spawn(move || follow_log_output(&log_file, follow_offset, output_stop));

        let input_result = forward_terminal_input_to_fifo(&args.input_path, Arc::clone(&stop));
        stop.store(true, Ordering::SeqCst);
...
#[cfg(unix)]
fn follow_log_output(
    log_file: &Path,
    mut offset: u64,
    stop: Arc<AtomicBool>,
) -> Result<(), String> {
    offset = offset.min(file_length(log_file).unwrap_or(0));
    while !stop.load(Ordering::Relaxed) {
        if log_file.exists() {
            match read_service_log_raw_delta(log_file, offset) {
                Ok((bytes, end_offset)) => {
                    offset = end_offset;
                    if !bytes.is_empty() {
                        let mut stdout = std::io::stdout().lock();
...
#[cfg(unix)]
fn forward_terminal_input_to_fifo(input_path: &Path, stop: Arc<AtomicBool>) -> Result<(), String> {
...
#[cfg(unix)]
fn set_stdin_raw_mode() -> Result<RawStdinGuard, String> {
    let stdin_fd = std::io::stdin().as_raw_fd();
...
#[cfg(unix)]
fn open_fifo_read_nonblocking(path: &Path) -> Result<libc::c_int, String> {
...
#[cfg(unix)]
fn write_all_fd(fd: libc::c_int, payload: &[u8]) -> Result<(), String> {
...
fn spawn_service_process_via_native_pty_runner(
    ...
    #[cfg(unix)]
    {
        // Keep the runner in a dedicated process group so stop/restart can
        // terminate it and its PTY child tree deterministically.
...
fn spawn_service_process(
    ...
    #[cfg(unix)]
    {
        // Isolate each service in its own process group so stop/restart can
...
#[cfg(not(unix))]
fn listening_pid_for_port(_bind_ip: &str, _port: u16) -> Option<u32> {
    None
}
...
#[cfg(not(unix))]
fn process_command_for_pid(_pid: u32) -> Option<String> {
    None
}
