...
Use standard process mode for services that need to run on Windows. Interactive tools that depend on a PTY should be run directly in a terminal until the Windows terminal runtime path is implemented.
